...
- Movie details include title, release year, runtime, synopsis, poster, up to
  ...
  may be missing.
  Missing or broken cast portraits keep the actor's name with a placeholder.
...
In **local mode**, movies (including selected TMDB metadata and per-language
detail caches), watched status, plans, draft fields, language, filters, theme,
and active list are stored under `movie-night:v1` in localStorage.
...
Party drafts (including unfinished date/place edits), theme, filters and view
...
Search terms, universe filters, page numbers, language, and TMDB IDs for selections
and saved-movie translation refreshes go through the configured Worker to TMDB.
Posters and cast portraits load directly from TMDB's image CDN, and the attribution
logo loads from TMDB.
