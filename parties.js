...
function applyPartySnapshot(snapshot, initial = false) {
  ...
  state = initial ? loadPartyView(snapshot) : { ...state, movies: snapshot.movies, plans: snapshot.plans };
...
function renderParty() {
  ...
  $("catalog-help").textContent = party.session
    ? "Busca un t\u00edtulo o descubre por universo. Elegir y guardar a\u00f1ade la pel\u00edcula a la lista compartida con tu nombre."
    : "Sin t\u00edtulo, descubre pel\u00edculas populares por universo. La b\u00fasqueda por t\u00edtulo incluye todos los universos. Elegir una peli la guarda en tu colecci\u00f3n.";
}
